...
Example release asset flow for the v2.0.0 unified toolkit tag:
...
The release workflow dispatches AUR sync only for normal semver tags like `v2.0.0`. Its `aur-sync` job only sends a `repository_dispatch` event with `event_type: aur-sync`; the real AUR push happens in the separate `zejtron - AUR Sync` workflow.

AUR sync can also be run manually with a `tag` input such as `v2.0.0` or `2.0.0`. It updates `aur/zejtron-bin/PKGBUILD`, regenerates `.SRCINFO`, commits as `rezky_nightky <rezky2399@proton.me>`, and pushes to `ssh://aur@aur.archlinux.org/zejtron-bin.git`.
